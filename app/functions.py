...
def deleteaccount_task():
    ...
    table_name = 'DeleteAccount'

    # Query to select user_id where deleted is False
    query = f"SELECT user_id FROM {table_name} WHERE deleted = 0"
    cursor.execute(query)
    rows = cursor.fetchall()

    # Extract user_id values and store them in a list
    user_ids = [row[0] for row in rows]

    # Print the list of user_ids
    print(user_ids)
...
scheduler.add_job(id='daily_task', func=deleteaccount_task, trigger='cron', hour=0, minute=0)
